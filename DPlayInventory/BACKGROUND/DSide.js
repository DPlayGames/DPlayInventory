global.DSide = OBJECT({
	...
	init : (inner, self) => {
		...
		Connector('DSide', (on, off, send) => {
			
			on('getTimeDiffWithNode', (notUsing, callback) => {
				callback(timeDiffWithNode);
			});
			
			// 특정 계정의 d 잔고를 가져옵니다.
			on('getDBalance', (notUsing, callback) => {
			...
			// 특정 계정의 d 잔고를 가져옵니다.
			on('saveAccountDetail', (params, callback) => {
				sendToNode('saveAccountDetail', params, callback);
			});
			
			// 계정 상세 정보를 가져옵니다.
			on('getAccountDetail', (accountId, callback) => {
				sendToNode('getAccountDetail', accountId, callback);
			});
			
			// 이름으로 계정을 찾습니다.
			on('findAccounts', (nameQuery, callback) => {
				sendToNode('findAccounts', nameQuery, callback);
			});
			
			// 친구 신청합니다.
			on('requestFriend', (targetAccountId, callback) => {
			...
			// 이미 친구 신청했는지 확인합니다.
			on('checkFriendRequested', (params, callback) => {
				sendToNode('checkFriendRequested', params, callback);
			});
			
			// 친구 신청자들의 ID를 가져옵니다.
			on('getFriendRequesterIds', (accountId, callback) => {
				sendToNode('getFriendRequesterIds', accountId, callback);
			});
			
			// 친구 요청을 거절합니다.
			on('denyFriendRequest', (requesterId, callback) => {
			...
			// 친구 요청을 수락합니다.
			on('acceptFriendRequest', (requesterId, callback) => {
			...
			// 친구들의 ID를 가져옵니다.
			on('getFriendIds', (accountId, callback) => {
				sendToNode('getFriendIds', accountId, callback);
			});
			
			// 두 유저가 친구인지 확인합니다.
			on('checkIsFriend', (params, callback) => {
				sendToNode('checkIsFriend', params, callback);
			});
			
			// 친구를 삭제합니다.
			on('removeFriend', (friendId, callback) => {
			...
			on('getGuildIdsByMemberCount', (notUsing, callback) => {
				sendToNode('getGuildIdsByMemberCount', undefined, callback);
			});
			
			// 이름으로 길드를 찾습니다.
			on('findGuilds', (nameQuery, callback) => {
				sendToNode('findGuilds', nameQuery, callback);
			});
			
			// 길드를 생성합니다.
			on('createGuild', (params, callback) => {
				sendToNode('createGuild', params, callback);
			});
			
			// 길드의 해시를 가져옵니다.
			on('getGuildHash', (guildId, callback) => {
				sendToNode('getGuildHash', guildId, callback);
			});
			
			// 길드 정보를 수정합니다.
			on('updateGuild', (params, callback) => {
				sendToNode('updateGuild', params, callback);
			});
			
			// 특정 계정이 가입한 길드 ID를 가져옵니다.
			on('getAccountGuildId', getAccountGuildId);
			
			
			// 특정 길드 정보를 가져옵니다.
			on('getGuild', getGuild);
			
			// 길드를 폐쇄합니다.
			on('removeGuild', (params, callback) => {
				sendToNode('removeGuild', params, callback);
			});
			
			// 길드 가입 신청합니다.
			on('requestGuildJoin', (targetGuildId, callback) => {
			...
			// 길드 가입 신청을 수락합니다.
			on('acceptGuildJoinRequest', (requesterId, callback) => {
			...
			// 길드에서 탈퇴합니다.
			on('leaveGuild', (notUsing, callback) => {
			...
			// 길드에서 내쫒습니다.
			on('banGuildMember', (accountId, callback) => {
			...
			on('login', (notUsing, callback) => {
				login(callback);
			});
			
			// 대상에 참여합니다.
			on('joinTarget', (params, callback) => {
			...
			on('getChatMessages', (target, callback) => {
			...
			on('getPendingTransactions', (target, callback) => {
